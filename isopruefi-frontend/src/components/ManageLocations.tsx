...
import {addPostalLocation, fetchPostalLocations, type PostalLocation, removePostalLocation} from "../api/clients";
    ...
    return (
        <div>
            ...
            <div className="mb-6 p-4 bg-gray-50 dark:bg-neutral-700 rounded-lg border dark:border-none">
                ...
                {msg && (
                    <div
                        className={`mt-3 text-sm font-medium ${/error|500|invalid/i.test(msg) ? "text-red-600" : "text-green-600"}`}>
                        {msg}
                    </div>
                )}
            </div>
                    ...
                    <div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {locations.map((location: any, index) => {
                                ...
                                const postalCode = location.postalCode || location.PostalCode || location.code || `unknown-${index}`;
                                const locationName = location.locationName || location.LocationName || location.name || location.location || 'Unknown Location';

                                return (
                                    <div
                                        key={`${postalCode}-${locationName}-${index}`}
                                    ...
                                    >
                                        <div className="flex-1">
                                            <span className="font-semibold text-gray-900 text-sm">{postalCode}</span>
                                            <span className="text-gray-600 text-sm ml-2">{locationName}</span>
                                        </div>
                                        <button
                                            onClick={() => handleRemove(typeof postalCode === 'number' ? postalCode : parseInt(postalCode))}
                                            disabled={busy}
